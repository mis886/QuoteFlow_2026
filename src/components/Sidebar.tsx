...
import { LayoutDashboard, FileText, FileSignature, ShoppingCart, Users, LineChart, Settings, Boxes, LogOut, Phone, Brain, ChevronRight, ChevronLeft, Gauge, FlaskConical, History, Truck, LifeBuoy, Warehouse, ArrowLeftRight } from 'lucide-react';
    ...
    <aside className={cn(
      ...
      <div className="flex-1 py-3 overflow-y-auto overflow-x-hidden">
        ...
        <div className="mb-4">
          ...
          <NavItem to="/history" icon={<History size={15} />} label="History" active={isActive('/history')} collapsed={collapsed} />
        </div>
      </div>
      ...
      <div className={cn(
        ...
        {collapsed ? (
          <>
            ...
            <button
              type="button"
              onClick={() => { if (confirm('Logout from Himalaya TerpenesEQ?')) logout(); }}
              title="Sign Out"
